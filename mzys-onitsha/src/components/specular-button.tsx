...
const SpecularButton: React.FC<SpecularButtonProps> = ({
  ...
  useEffect(() => {
    ...
    const update = (now: number) => {
      ...
      program.uniforms.uIntensity.value = ((p.intensity as number) ?? 1) * bright;
      program.uniforms.uShineSize.value = (((p.shineSize as number) ?? 10) * Math.PI) / 180;
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className={`relative m-0 inline-flex cursor-pointer items-center justify-center border-none font-medium leading-none tracking-[0.01em] outline-none transition-transform duration-150 active:scale-[0.97] disabled:cursor-default disabled:opacity-55 disabled:active:scale-100 whitespace-nowrap [color:var(--sb-text-color)] [border-radius:var(--sb-radius)] [background:color-mix(in_srgb,var(--sb-tint)_calc(var(--sb-tint-opacity)*100%),transparent)] [backdrop-filter:blur(var(--sb-blur))] shadow-[inset_0_1px_0_rgba(255,255,255,0.04),0_8px_24px_rgba(0,0,0,0.25)] focus-visible:outline-2 focus-visible:outline-offset-[3px] ${SIZES[size] || SIZES.md}${className ? ` ${className}` : ''}`}
      style={{
        '--sb-radius': `${radius}px`,
        '--sb-tint': tint,
    ...
    >
      <span ref={fxRef} aria-hidden="true" className="pointer-events-none absolute inset-0 z-[1] rounded-[inherit]" />
